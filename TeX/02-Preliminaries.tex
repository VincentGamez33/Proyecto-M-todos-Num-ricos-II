...
\begin{definition}{Métricas de Error Numérico}{errores}
    Sean $v_{real} = \fdif{f}{c}$ el valor de referencia (calculado analíticamente o con alta precisión) y $v_{aprox}$ el valor numérico obtenido. Definimos:
    \begin{enumerate}
        \item \textbf{Error Absoluto ($e_n$):} La magnitud de la diferencia directa.
            \[ e_n = \abs{v_{real} - v_{aprox}}. \]
        \item \textbf{Error Normalizado ($e_{N,h}$):} Medida utilizada para verificar la constante asintótica de convergencia $C$. Dado que utilizamos un método de orden 4, esperamos que:
            \[ e_{N,h} = \frac{e_n}{h^4} \le C. \]
        \item \textbf{Error Relativo ($e_{r}$):} Proporción del error respecto a la magnitud de la aproximación, útil para dimensionar el error en función de la escala de los datos (víctimas) \cite{DatosSESNSP2025}.
            \[ e_{r} = \frac{e_n}{\abs{v_{aprox}}}. \]
    \end{enumerate}
\end{definition}
